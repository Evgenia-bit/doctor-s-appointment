...
const addSlot = async (req: Request, res: Response) => {
    ...
    if (slot.length !== 19 &&  isNaN(Date.parse(slot))) {
        return res.status(422).json({ message: `Слот ${slot} введён в неверном формате. Все даты должны быть записаны в формате YYYY-MM-DDThh:mm:ss` })
    }
    if(doctor.slots.includes(slot)) {
        return res.json({ message: `Слот ${slot} уже существует у данного доктора` })
    }
...
const deleteSlot = async (req: Request, res: Response) => {
    ...
    if(!doctor.slots.includes(slot)) {
        return res.json({ message: `Слот ${slot} не существует у данного доктора` })
    }
